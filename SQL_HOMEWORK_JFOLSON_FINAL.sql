...
CREATE VIEW TOP_5_GENRES_BY_GROSS_REVENUE AS -- set the view title in its creation
SELECT category.name, sum(payment.amount) -- query tables
FROM category
JOIN film_category  ON film_category.category_id=category.category_id
JOIN inventory ON  film_category.film_id = inventory.film_id
JOIN rental ON rental. inventory_id = inventory.inventory_id 
JOIN payment ON rental. rental_id =payment.rental_id
GROUP BY category.name
ORDER BY sum(payment.amount) DESC   -- order by descending
LIMIT 5; -- show only 5  didnt try using SELECT TOP.. 
